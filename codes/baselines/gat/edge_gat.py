...
class GatEncoder(Net):
    ...
    def __init__(self,model_config, shared_embeddings=None):
        ...
        if model_config.embedding.dim != model_config.graph.node_dim:
            raise NotImplementedError("node dim must be equal to embedding dim")
        ...
        self.att1 = EdgeGatConv(model_config.graph.node_dim, model_config.graph.message_dim,
                                model_config.graph.edge_dim, heads=model_config.graph.num_reads, dropout=0.6)
        self.att2 = EdgeGatConv(model_config.graph.message_dim, model_config.graph.node_dim, model_config.graph.edge_dim)
